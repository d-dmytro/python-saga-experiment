...
def run_subscription(
    ...
    while(True):
        try:
            connection = BlockingConnection(ConnectionParameters(host="localhost", heartbeat=10, blocked_connection_timeout=10))
            ...
            for method_frame, properties, body in channel.consume(queue=queue_name, inactivity_timeout=1):
                ...
                try:
                    ...
                    print("failed to process a message with delivery tag %d" % (method_frame.delivery_tag))
                    print("exception:", e)
                    continue
